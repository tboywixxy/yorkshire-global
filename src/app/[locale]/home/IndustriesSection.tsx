// src/app/[locale]/IndustriesSection.tsx
"use client";
...
export default function IndustriesSection() {
  ...
  return (
    <section className="-mt-[10px] py-14 sm:py-16" aria-label={t("industries.aria.section")}>
      <Container>
        <SectionHeading title={t("industries.headingTitle")} />

        <div className="relative mt-6">
          <div
            className="pointer-events-none absolute left-0 top-0 z-10 h-full w-12"
            style={{
              background: "linear-gradient(to right, rgb(var(--background)) 0%, rgba(0,0,0,0) 100%)",
            }}
          />
          <div
            className="pointer-events-none absolute right-0 top-0 z-10 h-full w-12"
            style={{
              background: "linear-gradient(to left, rgb(var(--background)) 0%, rgba(0,0,0,0) 100%)",
            }}
          />
          ...
          <div
            ...
            role="region"
            aria-label={t("industries.aria.marquee")}
          >
            <div ref={trackRef} className="flex flex-nowrap gap-4 will-change-transform" style={{ transform: "translate3d(0,0,0)" }}>
              {items.map((industry, idx) => (
                ...
                >
                  <p className="text-sm font-semibold text-[rgb(var(--foreground))]">{industry}</p>
                  <p className="mt-2 text-sm text-[rgb(var(--muted))]">{t("industries.cardSubtitle")}</p>
          ...
          <p className="mt-3 text-xs text-[rgb(var(--muted))]">{t("industries.tip")}</p>
        </div>
      </Container>
    </section>
  );
