// src/app/[locale]/about/CapabilitiesSection.tsx
"use client";
...
const ACCENTS: Accent[] = [
  { key: "slate", bg: "rgba(241,245,249,0.80)", border: "rgba(100,116,139,0.35)", text: "rgb(15,23,42)", ring: "rgba(100,116,139,0.22)", stroke: "rgba(100,116,139,0.65)", chipBg: "rgba(241,245,249,0.95)" },
  { key: "zinc", bg: "rgba(250,250,250,0.90)", border: "rgba(113,113,122,0.35)", text: "rgb(24,24,27)", ring: "rgba(113,113,122,0.22)", stroke: "rgba(113,113,122,0.65)", chipBg: "rgba(244,244,245,0.95)" },
  { key: "stone", bg: "rgba(250,250,249,0.90)", border: "rgba(120,113,108,0.35)", text: "rgb(28,25,23)", ring: "rgba(120,113,108,0.22)", stroke: "rgba(120,113,108,0.65)", chipBg: "rgba(245,245,244,0.95)" },
  { key: "teal", bg: "rgba(240,253,250,0.85)", border: "rgba(13,148,136,0.28)", text: "rgb(0,72,63)", ring: "rgba(13,148,136,0.18)", stroke: "rgba(13,148,136,0.55)", chipBg: "rgba(204,251,241,0.80)" },
  { key: "sky", bg: "rgba(240,249,255,0.85)", border: "rgba(2,132,199,0.26)", text: "rgb(7,89,133)", ring: "rgba(2,132,199,0.18)", stroke: "rgba(2,132,199,0.55)", chipBg: "rgba(224,242,254,0.85)" },
  { key: "indigo", bg: "rgba(238,242,255,0.86)", border: "rgba(79,70,229,0.22)", text: "rgb(49,46,129)", ring: "rgba(79,70,229,0.16)", stroke: "rgba(79,70,229,0.50)", chipBg: "rgba(224,231,255,0.88)" },
  { key: "emerald", bg: "rgba(236,253,245,0.86)", border: "rgba(5,150,105,0.22)", text: "rgb(6,95,70)", ring: "rgba(5,150,105,0.16)", stroke: "rgba(5,150,105,0.50)", chipBg: "rgba(209,250,229,0.86)" },
  { key: "amber", bg: "rgba(255,251,235,0.88)", border: "rgba(217,119,6,0.22)", text: "rgb(120,53,15)", ring: "rgba(217,119,6,0.16)", stroke: "rgba(217,119,6,0.50)", chipBg: "rgba(254,243,199,0.88)" },
];
...
export default function CapabilitiesSection() {
  ...
  const capabilities: Capability[] = useMemo(
    () => [
      { title: t("capabilities.items.0.title"), description: t("capabilities.items.0.description") },
      { title: t("capabilities.items.1.title"), description: t("capabilities.items.1.description") },
      { title: t("capabilities.items.2.title"), description: t("capabilities.items.2.description") },
      { title: t("capabilities.items.3.title"), description: t("capabilities.items.3.description") },
      { title: t("capabilities.items.4.title"), description: t("capabilities.items.4.description") },
      { title: t("capabilities.items.5.title"), description: t("capabilities.items.5.description") },
      { title: t("capabilities.items.6.title"), description: t("capabilities.items.6.description") },
      { title: t("capabilities.items.7.title"), description: t("capabilities.items.7.description") },
    ],
    [t]
  );
  ...
  return (
    <section className="py-14 sm:py-16">
      <Container>
        <SectionHeading title={t("capabilities.headingTitle")} subtitle={t("capabilities.headingSubtitle")} />
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <p className="text-sm font-semibold text-[rgb(var(--foreground))]">{cap.title}</p>
                  <span
                    className="shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold"
                    style={{ backgroundColor: a.chipBg, color: a.text }}
                  >
                    {t("capabilities.focusChip")}
                  </span>
                </div>

                <p className="mt-2 text-sm leading-relaxed text-[rgb(var(--muted))]">{cap.description}</p>
              </div>
            );
          })}
        </div>
        ...
        {/* DESKTOP */}
        <div className="relative mt-8 hidden lg:block" ref={gridRef}>
          <svg
            ...
            <div className="space-y-2 lg:flex lg:flex-col lg:justify-center">
              {left.map((cap) => (
                <DesktopCard
                  key={cap.title}
                  cap={cap}
                  selected={active?.title === cap.title}
                  ...
                  {!active ? (
                    <div className="w-full text-center">
                      <p className="text-sm font-semibold text-[rgb(var(--foreground))]">
                        {t("capabilities.centerEmptyTitle")}
                      </p>
                      <p className="mt-2 text-sm text-[rgb(var(--muted))]">
                        {t("capabilities.centerEmptySubtitle")}
                      </p>
                    </div>
                  ) : (
                    <div className="w-full text-center">
